                ...
                <div class="x_panel">
                    <div class="x_content">
                        ...
                        if ($this->uri->segment(4) != 'none') {
                            $url = "/index/board/q/gubun/" . $this->uri->segment(4) . "/text/" . $this->uri->segment(5) . "/page/" . $this->uri->segment(6) . "";
                        } else {
                            $url = "/index/board/page/" . $this->uri->segment(6) . "";
                        }
                        ?>

                        <button type="button" class="btn btn-primary" onclick="location.href = '<?= $url ?>'">목록</button>
                        <?php if ($this->session->userdata('BRANCH_IDX') == '1') { ?>
                            <button type="button" class="btn btn-warning" onclick="location.href = '/index/board_modify/<?= $this->uri->segment(3) ?>/<?= $this->uri->segment(4) ?>/<?= $this->uri->segment(5) ?>/<?= $this->uri->segment(6) ?>'">수정</button>
                            <button type="button" id="del_contents_btn" class="btn btn-danger" value="<?= $info->BOARD_IDX ?>">삭제</button>
                        <?php } ?>
                        <!-- end project list -->
                    </div>
                </div>
