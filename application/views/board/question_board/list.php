                ...
                <div class="x_panel">
                    <div class="x_content">
                            ...
                            <tbody>
                                ...
                                foreach ($lists as $row) {
                                    ...
                                    <tr>
                                        <td><?= $num ?></td>
                                        <td>
                                            <a href="/index/question_board_view/<?= $row['QUESTION_BOARD_IDX'] ?>/<?= $gubun ?>/<?= $title ?>/<?= $page ?>"><?= $row['TITLE'] ?></a>
                                        </td>
                                        <td><?= $row['NAME'] ?></td>
                                        <td><?= $row['INS_DATE'] ?></td>
                                    </tr>
                        ...
                        <?php if ($this->session->userdata('BRANCH_IDX') !== '1') { ?>
                            <button type="button" class="btn btn-primary" onclick="location.href = '/index/question_board_write'">글 쓰기</button>
                        <?php } ?>
                        <!-- end project list -->
                    </div>
                </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        $("#search_text").keydown(function (key) {
            ...
            if (key.keyCode == 13) {
                if (text !== 'none') {
                    location.href = '/index/question_board/q/gubun/' + gubun + '/text/' + text + '';
                } else {
